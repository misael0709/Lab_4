...
public class SimulationEngine {
    ...
    public SimulationEngine(ParkConfig config) {
        ...
        for (int i = 0; i < config.getInt("workers.technicians", 2); i++) {
            technicians
                    .add(new Technician(i + 1, "Técnico " + (i + 1), config.getDouble("workers.dailySalary", 150.0)));
        }
    ...
    private List<Dinosaur> initDinosaurs(ParkConfig config) {
        ...
        for (int i = 0; i < carnivores; i++)
            list.add(new CarnivoreDinosaur(idCounter++, "T-Rex " + idCounter, "Tyrannosaurus"));

        int herbivores = config.getInt("dinosaurs.herbivores", 15);
        for (int i = 0; i < herbivores; i++)
            list.add(new HerbivoreDinosaur(idCounter++, "Triceratops " + idCounter, "Triceratops"));
        return list;
    }
    ...
    private List<Vehicle> initVehicles(ParkConfig config) {
        ...
        for (int i = 0; i < count; i++)
            list.add(new Vehicle(repairSteps));
        return list;
    }
    ...
    public void run() {
        ...
        // Llama al banner de inicio
        ParkMonitor.printStartBanner(config);
        // Imprime el step 0 (estado inicial)
        ...
        for (int step = 1; step <= totalSteps; step++) {
            ...
            for (Tourist t : new ArrayList<>(state.getActiveTourists())) {
                ...
                if (r < 0.5)
                    basicEnclosure.visit(t, rng, state.getDb());
                else if (r < 0.8)
                    premiumEnclosure.visit(t, rng, state.getDb());
                else
                    vipEnclosure.visit(t, rng, state.getDb());
            }
        ...
        ParkMonitor.printEndSummary(state);
        state.getDb().close();
    }
}